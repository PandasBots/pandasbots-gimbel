...
User will input the website URL and the program will return a JSON file wiht:
...
def main(url):
    '''
    From a product URL it will return a Json file with all the information.
    It also will save the info table in a dataframe, a json file and associate images.
    '''
    print("\n##### SYSTEM START #####\n")
    print("\nSystem running, please wait....")
    # Create object of class Webscrap
    product = Webscrap(url)
    # Gets data
    data = product.get_elements()
    # Save Data
    product.save_data(data)
    # Open and Print Data
    data = product.open_data()
    print(data)
    print("\n##### SYSTEM END #####\n")
    return data
